...
- **Four bridge failures, four contradicting remedies.** Timeout, unreachable, write-queue-wait and auth must never share a sentence: one forbids re-sending, one asks for it, one sends the reader to `check_setup`, and one says the panel refused on purpose and will again. → [bridge](docs/fragile-areas-bridge.md)
- **The bridge is authenticated, and CORS is not what does it.** Every route that reaches After Effects checks a per-session token the panel writes to `~/.engineroom-ae-mcp/token-<port>`; `/health` stays open for discovery. Dropping the `Access-Control-Allow-Origin` header would not have closed issue #106 on its own — a `text/plain` POST is a simple request, so it is never preflighted and the side effect lands whether or not the page can read the reply. → [bridge](docs/fragile-areas-bridge.md)
