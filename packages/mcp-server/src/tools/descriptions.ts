...
export const descriptions: Record<string, string> = {
  ...
  // ---------- raw ----------
  run_jsx:
    "Escape hatch: arbitrary ExtendScript in an undo group. Pass `code` inline, or `scriptPath` (absolute) to run a .jsx file the server reads — use that for anything long, so the script never enters the conversation. `libraries` (absolute .jsx paths) are read by the server too and inlined ahead of the script in the same scope, so their functions are callable from it; they are re-evaluated on every call, so keep them to declarations rather than to work. Keep shared helpers there instead of pasting them into every script. " +
